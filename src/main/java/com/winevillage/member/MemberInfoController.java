...
public class MemberInfoController {
	...
	@ModelAttribute("memberCartCount")
	public Integer getMemberCartCount(HttpServletRequest request) {
		HttpSession session = request.getSession(); // 세션이 없으면 null 반환
		...
		if (session != null) {
			...
			if (securityContext != null) {
				...
				if (authentication != null && authentication.isAuthenticated()) {
					...
					String query = "SELECT memberCartCount FROM member WHERE memberid = ?";
					return jdbcTemplate.queryForObject(query, new Object[]{memberId}, Integer.class);
				}
			}
		}
