...
import com.winevillage.note.CouponDTO;
import com.winevillage.note.ICouponService;
	...
	public Map<String, Object> joinSubmit(@RequestBody MemberDTO memberDTO,
		...
		try {
			...
			if (result > 0) {
				...
				//신규회원 가입 감사 포인트(멤버 테이블에 기록)
				memberDTO.setMemberPoints(100);
				memberDTO.setMemberId(authentication.getName());
				dao.addMemberPoints(memberDTO);
				
				//신규회원 가입 감사 포인트(마일리지 테이블에 기록)
				MileageDTO mileageDTO = new MileageDTO();
				mileageDTO.setMemberId(authentication.getName());
				mileageDTO.setMileage_name("신규회원 가입 감사포인트");
				mileageDTO.setPoints(100);
				mileage.plusMileage(mileageDTO);
				
				//신규회원 가입 감사 쿠폰(쿠폰 테이블에 기록)
				CouponDTO couponDTO = new CouponDTO();
				//coupon.searchCouponCode("WELCOME");
				couponDTO.setMemberId(authentication.getName());
				couponDTO.setCoupon_name("신규회원 가입 감사쿠폰");
				couponDTO.setDiscount(5000);
				couponDTO.setMinimum(50000);
				...
				coupon.addCoupon(couponDTO);
				
				//신규회원 가입 감사 쿠폰(멤버 테이블에 기록)
				dao.addCouponCount(authentication.getName());
			} else {
