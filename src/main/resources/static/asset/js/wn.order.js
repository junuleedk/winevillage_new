...
order.CalcInfo = function () {
	...
	var const_trans_limit = $("#const_trans_limit").val().onlyNum();		//배송비 제한금액
	...
	var use_giftcard	  = $("#use_giftcard").val().onlyNum();			//기프트카드 사용금액
	...
	//최종 할인합계액
	tot_use_discount = coupon_sale + use_reserve + use_deposit + use_giftcard;

	//최종 할인적용가
	tot_price = tot_price - coupon_sale;

	if (coupon_gb == "2" || free_trans_yn == "Y") {
		...
		if( add_trans_yn == "Y" ){
			...
			if( tot_price >= const_trans_limit && const_trans_limit > 0){
				deliverypay = 0;
			}
		}
	}
	...
	//배송비 및 적립금 예치금 적용
	tot_price = tot_price - use_reserve - use_deposit - use_giftcard + deliverypay;
	...
	//기프트카드 사용액
	$("#giftcard_price_dd").html("- "+Functions.NumberFormat(use_giftcard)+"원");
...
order.initOrder = function (){
	...
	$("#use_reserve").blur(function (){
		...
		if( parseInt($("#coupon_sale").val().onlyNum()) > 0 ){
			...
			if( $("#deposit_price_dd").size() > 0){
				var use_deposit = $("#use_deposit").val().onlyNum(); 					// 예치금 할인금액
			} else {
				var use_deposit = 0;
			}

			var this_val = $(this).val().onlyNum(); 								// 입력된 현재금액
			var max_use_reserve = order_origin_price - coupon_sale - use_deposit - use_giftcard;
			if( $(this).val() == "" ){
				order.ReserveReSet();
				return;
			}
			...
			**/
			...
			//순수한 적립금 사용가능액
			var tot_sale_price = this_val + coupon_sale + use_deposit + use_giftcard;
	...
	$("#use_deposit").blur(function (){
		...
		var this_val = $(this).val().onlyNum(); 								// 입력된 현재금액
		var max_use_deposit = order_origin_price - coupon_sale - use_reserve - use_giftcard;
		if( $(this).val() == "" ){
			order.depositReSet();
			return;
		}
		...
		if( parseInt($("#coupon_sale").val().onlyNum()) > 0 ){
			...
			if($(this).is(":checked") && ($("#mb_reserve").val()>=1000) ){
				...
				if(mb_reserve >= order_origin_price){
					$("#use_reserve").val(Functions.NumberFormat(order_origin_price - coupon_sale - use_deposit - use_giftcard));
				}else{
					if(coupon_sale+mb_reserve+use_deposit+use_giftcard < order_origin_price ){
